...
class ExerciseVC : UIViewController, UIScrollViewDelegate  {
    ...
    override func viewDidLoad() {
       ...
       self.backBtn.addTarget(self, action: #selector(backButtonSelected), for: .touchUpInside)
        ...
        if UIDevice.current.type == .iPhone6 || UIDevice.current.type == .iPhone6S || UIDevice.current.type == .iPhone7 || UIDevice.current.type == .iPhone8 || UIDevice.current.type == .iPhone6Plus || UIDevice.current.type == .iPhone6SPlus || UIDevice.current.type == .iPhone7Plus || UIDevice.current.type == .iPhone8Plus  || UIDevice.current.type == .iPhone11Pro || UIDevice.current.type == .iPhone12mini || UIDevice.current.type == .iPhoneX || UIDevice.current.type == .iPhoneXS || UIDevice.current.type == .iPhoneXR {
        ...
        titleLbl.text = "Exercise Recommendations"
        ...
        let exerciseImageView = UIImageView(image: UIImage(named: "exerciseIcon"))
        ...
        let stringTxt = "The minimum exercise recommendations would be 30 minutes of moderate level cardio-based exercise per day. Additionally it does help to set yourself a step goal. The recommended daily step target it 10,000 steps. The key to avoiding a weight-loss plateau is adjusting the exercise goals and lowering calories slowly, until you’re able to reach a weight that is healthy and easy to maintain. If you increase your step target by 5-10% per fortnight you’ll naturally avoid plateaus. If you’re regularly training at the gym and performing at least 45 minutes of weight training 3-4 times per week, you’ll be absolutely on track with your exercise goals. The key with weight training is to keep the body guessing! The best way to achieve this is by changing your program every 6-8 weeks so that the body doesn’t have time to adapt to the routine and you can build lean muscle gains all over!"
        ...
        self.nxtBtn.setTitle("Get Cooking!", for: .normal)

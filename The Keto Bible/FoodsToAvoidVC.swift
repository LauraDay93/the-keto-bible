...
class FoodsToAvoidVC : UIViewController, UIScrollViewDelegate  {
    ...
    override func viewDidLoad() {
        ...
        if UIDevice().type == .iPhone5 || UIDevice().type == .iPhone5C || UIDevice().type == .iPhone5S || UIDevice().type == .iPhoneSE {
            self.bgVC = UIImageView(image: UIImage(named: "avoidScreen5"))
        } else if StyleManager.isIpad() {
            self.bgVC = UIImageView(image: UIImage(named: "avoidIpad"))
        } else {
            self.bgVC = UIImageView(image: UIImage(named: "foodsToAvoidScreen"))
        }

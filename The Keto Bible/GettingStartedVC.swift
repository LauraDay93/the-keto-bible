...
class GettingStartedVC : UIViewController, UIScrollViewDelegate  {
    ...
    override func viewDidLoad() {
       ...
       self.backBtn.addTarget(self, action: #selector(backButtonSelected), for: .touchUpInside)
        ...
        } else if UIDevice.current.type == .iPhone6Plus || UIDevice.current.type == .iPhone6SPlus || UIDevice.current.type == .iPhone7Plus || UIDevice.current.type == .iPhone8Plus || UIDevice.current.type == .iPhone11Pro || UIDevice.current.type == .iPhone12mini || UIDevice.current.type == .iPhoneX || UIDevice.current.type == .iPhoneXS || UIDevice.current.type == .iPhoneXR {
        ...
        let stringTxt = "The key with “going keto” is making it through the initial stages of the diet. Some people report significant symptoms in the first week or so, of making severe carbohydrate restriction. Some of the common complaints are: headache, increased thirst, feeling sluggish, changes in bowel habit, changes to urine output and some loss in strength when training. I like to think of these symptoms as being similar to a hang over. If you drink too much alcohol, the next day your body is depleted of nutrients, your organs are working a bit harder than usual to filter everything through and you need a bit of rest because you’re physically recovering. Similarly, if you go from carbohydrate saturated diet and suddenly restrict, your body initially uses all stored carbohydrates first and then makes the gradual shift to using ketones for energy. Traditionally the ketogenic diet has been implemented starting with a period of fasting and fluid restriction that has been uncomfortable and complicated by the dehydration effects that ketosis has alone."
    ...
    @objc func nextBtnPressed() {
        self.nxtBtn.pop {
            self.navigationController?.pushViewController(GettingStartedVC2(), animated: true)
        }
    }
